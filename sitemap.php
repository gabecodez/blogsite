...
echo '<lastmod>' . $LAST_SITE_UPDATE . '</lastmod>';
...
$articles_sql = "SELECT articles.slug AS article_slug, articles.published_date, articles.last_modified_date, categories.slug AS category_slug 
        FROM articles 
...
if (!empty($articles)) {
    foreach ($articles as $article) {
        ...
        $last_modified = htmlspecialchars($article['last_modified_date'], ENT_XML1, 'UTF-8'); // Format date if necessary
...
$products_sql = "SELECT products.slug AS product_slug, products.name, products.meta_description, products.preview_image_ids, products.last_modified_date, shop_categories.slug AS category_slug
                FROM products 
...
if (!empty($products)) {
    foreach ($products as $product) {
        ...
        $last_modified = htmlspecialchars($product['last_modified_date'], ENT_XML1, 'UTF-8'); // Format date if necessary
